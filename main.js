    ...
    collectVars(chunk, pathParts, result) {
        ...
        while ((m = re.exec(chunk)) !== null) {
            ...
            if (!hasChild) {
                ...
                // Zwischenknoten: selbst auch speichern (z.B. "Vorrat")
                // aber nur wenn die URI eine "echte" Variable ist (letztes Segment != 0)
                const uriParts = uri.split('/').filter(Boolean);
                const lastSeg  = uriParts[uriParts.length - 1];
                if (lastSeg !== '0') {
                    const fullPath = [...pathParts, name].join('.');
                    result.push({ uri, name: fullPath });
                }
